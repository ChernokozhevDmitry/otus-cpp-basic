...
void Physics::update(std::vector<Dust>& dusts, std::vector<Ball>& balls, const size_t ticks) const {

    for (size_t i = 0; i < ticks; ++i) {
        move(balls);
        move(dusts);
        collideWithBox(balls);
        collideBalls(dusts, balls);
    }
}

void Physics::setdust(std::vector<Dust>& dusts, Ball& a) const {
    for (int i = -3; i <= 3; ++i) {
    Dust dust(a.getCenter(), Point(i*100,i*100), 50);
...
void Physics::collideBalls(std::vector<Dust>& dusts, std::vector<Ball>& balls) const {
    for (auto a = balls.begin(); a != balls.end(); ++a) {
        for (auto b = std::next(a); b != balls.end(); ++b) {
            ...
            if (distanceBetweenCenters2 < collisionDistance2) {
                processCollision(dusts, *a, *b, distanceBetweenCenters2);
            }
        }
    }
}
...
void Physics::processCollision(std::vector<Dust>& dusts, Ball& a, Ball& b, double distanceBetweenCenters2) const {
