...
### "Откатить" [**git add**](#подготовить-файлы-к-сохранению)

Созданный или измененный файл, добавленный «на коммит» (staging area) командой [**git add**](#подготовить-файлы-к-сохранению), можно вернуть в состояние [untracked/modified](#состояние-статусы-файлов-в-репозитарии):  
**git restore --staged \<file\>**  
Сбросить все файлы из staged обратно в [untracked/modified](#состояние-статусы-файлов-в-репозитарии) можно командой:  
**git restore --staged .**  
Она сбросит всю текущую папку (.)

### "Откатить" [**git commit**](#сделать-коммит-commit)
Если необходимо «откатить» то, что уже было [закоммичено](#сделать-коммит-commit), то есть вернуть состояние репозитория к более раннему, нужно воспользоваться командов:  
**git reset --hard \<commit hash\>**  
*«хеш коммита» (commit hash) будет виден, если воспользоваться командой [**git log --oneline**](#просмотреть-историю-коммитов)  
*Команду **git reset --hard** нужно применять осторожно! При удалении коммитов можно потерять что-то нужное.*

### "Откатить" изменения [**modified**](#состояние-статусы-файлов-в-репозитарии)
Cлучайно измененный файл, который отображается в *Changes not staged for commit (modified)*, можно вернуть до последней версии, которая была сохранена через [**git commit**](#сделать-коммит-commit) или [**git add**](#подготовить-файлы-к-сохранению):  
**git restore \<file\>**

### Смотреть изменения в файлах
**git diff** (изменения в закоммиченной копии и локально измененной)  
**git diff --staged** (изменения в staging area и локально измененной)  
**git diff \<commit hash1\> \<commit hash2\>** (изменения в 2х коммитах по хэш) 

### Клонирование репозитариев
